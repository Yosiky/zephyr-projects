...
K_HEAP_DEFINE(message_heap, 512);
...
#define PRODUCER_THREAD_DELAY      0
...
static void producer_task(void *, void *, void *)
{
	...
	while (sequence < PRODUCER_THREAD_CNT) {
		struct message *msg = k_heap_alloc(&message_heap, sizeof(*msg), K_NO_WAIT);
		...
		LOG_INF("Put message(%d) into FIFO", sequence);
		k_fifo_put(&message_fifo, msg);
		k_msleep(PRODUCER_THREAD_MSEC);
	}
}
